/**
 *
 * @type {{listToArray, jsonParse, win, offset, getCss, setCss, css, getByClass, hasClass, addClass, removeClass, toggle, children, prev, prevAll, next, nextAll, sibling, siblings, first, last, prepend, insertAfter}}
 */
//数组转化
var utils = (function () {
    ...
    function getCss(curEle, attr) {
        ...
        if ("getComputedStyle" in window) {
            val = window.getComputedStyle(curEle, null)[attr];//[attr]属性名
        } else { // IE Low filter
            if (attr === 'opacity') {//兼容问题
                // 'alpha(opacity=10)'
     ...
     */
    function hasClass(curEle, cn) {//\d边界问题
